...
def record():
    ...
    if st.session_state.subpagina == "tipo1":
        ...
        with st.form("form_tipo3"):
            op = st.radio(
                "¿Are we the Lender or Borrower?",
                ["Lender", "Borrower"],
                horizontal=True
            )
            date = st.date_input("Fecha")
            amount = st.number_input("Monto", format="%.2f")
            whom = st.text_input("To/From Whom?")
            interest=st.number_input("Interés", format="%.2f")

            guardar = st.form_submit_button("Save")
            value=(date, amount, whom, interest)
            if guardar:
                connection=mysql_connector_connection()
                mycursor=connection.cursor()
                qq.record_debt(op,value,mycursor,connection)
                close_con_cur(mycursor,connection)
                st.success("Saved")